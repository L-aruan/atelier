...
export function FileDropHero() {
  ...
  if (suggestedTools.length > 0) {
    ...
    const categoryLabel = category === 'image' ? '图片' : '文件';
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-6">
        <div className="flex items-center gap-2 mb-4">
          ...
          <span className="text-gray-700 text-sm">
            已识别 <strong>{droppedFiles.length} 个{categoryLabel}</strong>，以下工具可处理：
          </span>
          <button
            onClick={() => {
              setDroppedFiles([]);
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {suggestedTools.map((tool) => (
            <button
              ...
              className="bg-gray-50 border border-gray-200 rounded-lg p-3 text-center hover:border-blue-400 hover:bg-blue-50 transition-colors"
            >
